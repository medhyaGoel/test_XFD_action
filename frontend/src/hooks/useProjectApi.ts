...
export const useProjectApi = () => {
  ...
  const fetchProjectsByOrg = useCallback(async (orgId: string) => {
    setLoading(true);
    setError(null);
    try {
      const response: Project[] = await apiGet<Project[]>(`/projects?orgId=${orgId}`); 
      return response;
    } catch (err: any) {
  ...
  const createProject = useCallback(async (url : string, organizations: Organization[] ) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 403) {
        setError(`403 - Forbidden Error. Permission not granted to make this request.`);
        throw err; 
